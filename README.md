...
These codes contains the experimental codes of Structural Redundancy Evaluation for Convolutional Neural Networks via Task Contribution Analysis. It includes the structural redundancy evaluation code for the three-class and ten-class classification of VGGNet-16, ResNet-18, ResNet-32 and ResNet-56 in the experimental part of the paper. The codes runs on Tensorflow 2.7 and python 3.7.0, they also relies on pandas, scipy and libraries. The dataset used in this paper is CIFAR-10. 
...
The main part of the code is located in the "class-3" and "class-10" folders, while the plotting part is in the "plot" folder. The "class-3" folder contains the evaluation codes and results using the original training dataset and the distillation dataset, while the "class-10" folder only contains the evaluation codes and results using the distillation dataset. The evaluation results are all saved in ".json" files, while the evaluation code is in ".ipynb" files. The code consists of three types. The first type is data preparation code, usually including code related to data extraction and data distillation, and the names contain "data". The second type is the network training code. This code is used to train the network to be evaluated. It is named according to the network type, such as "VGG-16". The third type is for evaluating algorithm codes, and the names include "rude".
